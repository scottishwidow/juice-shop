...
void describe('keyServer path traversal', () => {
  let req: any
  let res: any
  let next: any
  ...
  // Express decodes params, so GET /encryptionkeys/%2e%2e yields params.file === '..',
  // which contains no '/' and passes the current guard.
  void it('should reject ".." rather than resolve and serve it', () => {
    req.params.file = '..'
  ...
  void it('should raise error for slashes in filename', () => {
    ...
    assert.equal(res.status.mock.calls[0].arguments[0], 403)
    assert.equal(next.mock.calls.length, 1)
    assert.equal(next.mock.calls[0].arguments[0].message, 'File names cannot contain forward slashes!')
  })
})
